...
def segment_func(midi_wav_args_zip):
    ...
    while (i+1)*segment_length_point < wav.shape[0]:
        ...
        wav_begin_point = int((args.num_segment_frame - OVERLAP_FRAME) * frame_shift_point * i)
        wav_end_point = int((args.num_segment_frame - OVERLAP_FRAME) * frame_shift_point * i + args.num_segment_frame * frame_shift_point + frame_length_point)
        wav_segment = wav[wav_begin_point:wav_end_point]
        midi_segment = midi[:, int((args.num_segment_frame-OVERLAP_FRAME)*i):int((args.num_segment_frame-OVERLAP_FRAME)*i+args.num_segment_frame)]
