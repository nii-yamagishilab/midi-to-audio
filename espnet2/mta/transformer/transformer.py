...
from espnet2.mta.gst.style_encoder import StyleEncoder
from espnet2.mta.transformer.encoder import Encoder


class Transformer(AbsTTS):
    """Transformer-TTS module.

    This is a module of midi-to-audio Transformer 
    similar with the TTS model described in `Neural Speech Synthesis
    ...
    def __init__(
        ...
        self.odim = odim
        # self.eos = idim - 1
        self.eos = -1    # NOTE(Xuan: set eos for midi2wav as -1.)
    ...
    def forward(
        ...
        Args:
            midi (LongTensor): Batch of padded character ids (B, Tmax).
            midi_lengths (LongTensor): Batch of lengths of each input batch (B,).
        ...
        """
        midi /= 127   # NOTE(Xuan: scaling to 0-1)
        midi = midi[:, : midi_lengths.max()]  # for data-parallel
        feats = feats[:, : feats_lengths.max()]  # for data-parallel
        batch_size = midi.size(0)

        # Add eos at the last of sequence
        xs = F.pad(midi, [0, 0, 0, 1], "constant", self.padding_idx)
        for i, l in enumerate(midi_lengths):
            xs[i, l] = self.eos
        ilens = midi_lengths + 1
        ...
        if self.use_guided_attn_loss:
            ...
            if "encoder" in self.modules_applied_guided_attn:
                ...
                for idx, layer_idx in enumerate(
                    ...
                    if idx + 1 == self.num_layers_applied_guided_attn:
                        break
                att_ws = torch.cat(att_ws, dim=1)  # (B, H*L, T_midi, T_midi)
            ...
            if "encoder-decoder" in self.modules_applied_guided_attn:
                ...
                for idx, layer_idx in enumerate(
                    ...
                    if idx + 1 == self.num_layers_applied_guided_attn:
                        break
                att_ws = torch.cat(att_ws, dim=1)  # (B, H*L, T_feats, T_midi)
    ...
    def inference(
        ...
        Args:
            midi (LongTensor): Input sequence of characters (T_midi,).
        ...
        Returns:
            Dict[str, Tensor]: Output dict including the following items:
                ...
                * att_w (Tensor): Source attn weight (#layers, #heads, T_feats, T_midi).
        ...
        # add eos at the last of sequence
        x /= 127
        x = F.pad(x, [0, 0, 0, 1], "constant", self.eos)
        ...
        if use_teacher_forcing:
            ...
            att_ws = torch.stack(att_ws, dim=1)  # (B, L, H, T_feats, T_midi)

            # NOTE Xuan: add feat_gt for debug
            return dict(feat_gen=outs[0], att_w=att_ws[0], feat_gt=ys[0])
        ...
        # concatenate attention weights -> (#layers, #heads, T_feats, T_midi)
        att_ws = torch.stack(att_ws, dim=0)
        return dict(feat_gen=outs, prob=probs, att_w=att_ws, feat_gt=feats)
