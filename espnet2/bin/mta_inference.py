...
from espnet2.mta.tacotron2 import Tacotron2
from espnet2.mta.transformer import Transformer
from espnet2.mta.utils import DurationCalculator
...
class Text2Speech:
    ...
    def __init__(
        ...
        # setup model
        model, train_args = MTATask.build_model_from_file(
        ...
        self.preprocess_fn = MTATask.build_preprocess_fn(train_args, False)
        self.use_teacher_forcing = use_teacher_forcing
        self.seed = seed
        self.always_fix_seed = always_fix_seed
        self.vocoder = None
        if self.tts.require_vocoder:
            vocoder = MTATask.build_vocoder_from_file(
    ...
    def __call__(
        self,
        midi: Union[str, torch.Tensor, np.ndarray],
        ...
        assert check_argument_types()
        # check inputs
        if self.use_speech and audio is None:
        ...
        if isinstance(midi, str):
            midi = self.preprocess_fn("<dummy>", dict(midi=midi))["midi"]
        batch = dict(midi=midi)
        if audio is not None:
            batch.update(audio=audio)
        ...
        if self.vocoder is not None:
            if output_dict.get("feat_gen_denorm") is not None:
                input_feat = output_dict["feat_gen_denorm"]  # here
            else:
...
def inference(
    ...
    loader = MTATask.build_streaming_iterator(
        data_path_and_name_and_type,
        dtype=dtype,
        batch_size=batch_size,
        key_file=key_file,
        num_workers=num_workers,
        preprocess_fn=MTATask.build_preprocess_fn(text2speech.train_args, False),
        collate_fn=MTATask.build_collate_fn(text2speech.train_args, False),
    ...
    (output_dir / "gt_denorm").mkdir(parents=True, exist_ok=True)
    (output_dir / "speech_shape").mkdir(parents=True, exist_ok=True)
    (output_dir / "wav").mkdir(parents=True, exist_ok=True)
    (output_dir / "feat_gt").mkdir(parents=True, exist_ok=True)
    (output_dir / "feat_gen").mkdir(parents=True, exist_ok=True)
    (output_dir / "feat_gen_denorm").mkdir(parents=True, exist_ok=True)
    ...
    with NpyScpWriter(
        ...
        output_dir / "gt", output_dir / "gt/feats.scp"
    ) as gt_writer, NpyScpWriter(
        output_dir / "gt_denorm", output_dir / "gt_denorm/feats.scp"
    ) as gt_denorm_writer, open(
        ...
        for idx, (keys, batch) in enumerate(loader, 1):
            ...
            if output_dict.get("feat_gen") is not None:
                ...
                # Plot midi-fbank spectrogram
                midi_spec_gen = output_dict["feat_gen"].cpu().numpy().T
                fig = plt.Figure()
                ax = fig.add_subplot(1, 1, 1)
                midi_gen_fig = ax.pcolormesh(midi_spec_gen.astype(np.float32), cmap='viridis')
                fig.colorbar(midi_gen_fig, ax=ax, format='%+2.0f dB')
                ax.set_title(f"{key}")
                ax.set_xlabel("time: frame")
                fig.set_tight_layout(True)
                fig.savefig(output_dir / f"feat_gen/{key}.png")
                fig.clf()

                # Plot midi-fbank spectrogram denorm
                midi_spec_denorm = output_dict["feat_gen_denorm"].cpu().numpy().T
                fig = plt.Figure()
                ax = fig.add_subplot(1, 1, 1)
                midi_denorm_fig = ax.pcolormesh(midi_spec_denorm.astype(np.float32), cmap='viridis')
                fig.colorbar(midi_denorm_fig, ax=ax, format='%+2.0f dB')
                ax.set_title(f"{key}")
                ax.set_xlabel("time: frame")
                fig.set_tight_layout(True)
                fig.savefig(output_dir / f"feat_gen_denorm/{key}.png")
                fig.clf()

                if output_dict.get("feat_gt") is not None and output_dict.get("feat_gt_denorm") is not None:
                    gt_writer[key] = output_dict["feat_gt"].cpu().numpy()
                    gt_denorm_writer[key] = output_dict["feat_gt_denorm"].cpu().numpy()
                    # Plot midi-fbank spectrogram ground truth
                    midi_spec_gt = output_dict["feat_gt"].cpu().numpy().T
                    fig = plt.Figure()
                    ax = fig.add_subplot(1, 1, 1)
                    midi_gt_fig = ax.pcolormesh(midi_spec_gt.astype(np.float32), cmap='viridis')
                    fig.colorbar(midi_gt_fig, ax=ax, format='%+2.0f dB')
                    ax.set_title(f"{key}")
                    ax.set_xlabel("time: frame")
                    fig.set_tight_layout(True)
                    fig.savefig(output_dir / f"feat_gt/{key}.png")
                    fig.clf()
    ...
    if output_dict.get("feat_gen") is None:
        ...
        shutil.rmtree(output_dir / "feat_gen")
    if output_dict.get("feat_gen_denorm") is None:
        shutil.rmtree(output_dir / "denorm")
        shutil.rmtree(output_dir / "feat_gen_denorm")
    if output_dict.get("feat_gt") is None:
        shutil.rmtree(output_dir / "gt")
        shutil.rmtree(output_dir / "gt_denorm")
        shutil.rmtree(output_dir / "feat_gt")
