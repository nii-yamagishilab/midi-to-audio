...
class Text2Speech:
    ...
    def __init__(
        ...
        # setup model
        model, train_args = GANMTATask.build_model_from_file(
        ...
        self.preprocess_fn = GANMTATask.build_preprocess_fn(train_args, False)
        self.use_teacher_forcing = use_teacher_forcing
        self.seed = seed
        self.always_fix_seed = always_fix_seed
        self.vocoder = None
        if self.tts.require_vocoder:
            vocoder = GANMTATask.build_vocoder_from_file(
...
def inference(
    ...
    loader = GANMTATask.build_streaming_iterator(
        data_path_and_name_and_type,
        dtype=dtype,
        batch_size=batch_size,
        key_file=key_file,
        num_workers=num_workers,
        preprocess_fn=GANMTATask.build_preprocess_fn(text2speech.train_args, False),
        collate_fn=GANMTATask.build_collate_fn(text2speech.train_args, False),
    ...
    (output_dir / "timbre_feature").mkdir(parents=True, exist_ok=True)
    ...
    with NpyScpWriter(
        ...
        output_dir / "timbre", output_dir / "timbre/feats.scp"
    ) as timbre_writer, open(
        ...
        for idx, (keys, batch) in enumerate(loader, 1):
            ...
            if output_dict.get("feat_gen") is not None:
                ...
                if output_dict.get("timbre_feature") is not None:
                    timbre_writer[key] = output_dict["timbre_feature"].cpu().numpy()
                ...
                if output_dict.get("feat_gt") is not None:
