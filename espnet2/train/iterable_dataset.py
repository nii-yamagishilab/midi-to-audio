...
def load_midi_npz(input):
    sparse_matrix = scipy.sparse.load_npz(input)
    midi = sparse_matrix.todense().T   # (Time, Dim) 
    return midi


def load_mfbsp(input, col=128, format='f4',end='l'):
    f = open(input, 'rb')
    if end=='l':
        format = '<'+format
    elif end=='b':
    ...
    if data.ndim == 2 and data.shape[1] == 1:
        ...
        return data
    # TODO (Xuan): confirm the output dim


def load_audio(input):
    array, rate = librosa.core.load(input, sr=24000)
    # array -= array.min()
    # array *= 2.0/array.max()
    # array -= 1.0
    return array
