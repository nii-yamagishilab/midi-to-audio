...
from espnet2.fileio.npz_scp import NpzScpReader
...
from espnet2.fileio.audio_scp import AudioScpReader
from espnet2.fileio.embed_scp import EmbedScpReader, MeanEmbedScpReader
...
def audio_loader(path, float_dtype=None):
    # The file is as follows:
    #   utterance_id_A /some/where/a.wav
    #   utterance_id_B /some/where/a.flac
    ...
    # like Kaldi e.g. "cat a.wav |".
    # NOTE(kamo): The audio signal is normalized to [-1,1] range.
    loader = AudioScpReader(path, normalize=True, always_2d=False)
...
def embed_loader(feat, float_dtype=None):
    loader = EmbedScpReader(feat, normalize=True, always_2d=False)
    return AdapterForSoundScpReader(loader, float_dtype)


def mean_embed_loader(feat, float_dtype=None):
    loader = MeanEmbedScpReader(feat, normalize=True, always_2d=False)
    return AdapterForSoundScpReader(loader, float_dtype)
...
DATA_TYPES = {
    ...
    "npz": dict(
        func=NpzScpReader,
        kwargs=[],
        help="Npz file format."
        "\n\n"
        "   utterance_id_A /some/where/a.npz\n"
        "   utterance_id_B /some/where/b.npz\n"
        "   ...",
    ),
    "audio": dict(
        func=audio_loader,
    ...
    "embed": dict(
        func=embed_loader,
        kwargs=["float_dtype"],
        help="Speaker Embedding format types wicth supported by scp"
        "\n\n"
        "   utterance_id_a embedding_of_utt_a\n"
        "   utterance_id_b embedding_of_utt_b\n"
        "   ...",
    ),
    "mean_embed": dict(
        func=mean_embed_loader,
        kwargs=["float_dtype"],
        help="Speaker Embedding format types wicth supported by scp"
        "\n\n"
        "   utterance_id_a embedding_of_utt_a\n"
