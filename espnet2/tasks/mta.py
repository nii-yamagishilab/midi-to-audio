...
from espnet2.mta.espnet_model import ESPnetTTSModel
...
from espnet2.mta.feats_extract.dio import Dio
from espnet2.mta.feats_extract.energy import Energy
from espnet2.mta.feats_extract.linear_spectrogram import LinearSpectrogram
from espnet2.mta.feats_extract.log_mel_fbank import LogMelFbank
from espnet2.mta.feats_extract.log_midi_fbank import LogMIDIFbank
from espnet2.mta.feats_extract.log_spectrogram import LogSpectrogram
from espnet2.mta.tacotron2 import Tacotron2
from espnet2.mta.transformer import Transformer
from espnet2.mta.utils import ParallelWaveGANPretrainedVocoder
...
# Xuan: I commented the vits and joint_text2wav, becuase I do not use them.
...
class MTATask(AbsTask):
    ...
    def add_task_arguments(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument(
            ...
            type=str,
            default="phn",
            choices=["bpe", "char", "word", "phn", "proll"],
    ...
    def required_data_names(
        ...
        if not inference:
            ...
            # Inference mode
            # retval = ("midi") # NOTE(Xuan): I think it should be this.
            retval = ("midi", "audio")
        return retval
    ...
    def optional_data_names(
        ...
        if not inference:
            ...
            # Inference mode
            # retval = ("spembs", "audio", "durations", "sids", "lids") # NOTE(Xuan): I think it should be this.
            retval = ("spembs", "durations", "sids", "lids")
        return retval

    @classmethod
    def build_model(cls, args: argparse.Namespace) -> ESPnetTTSModel:
        assert check_argument_types()
        # Xuan: we do not need token_list in midi-to-wav.
        """
        ...
        """
        vocab_size = 128  # number of midi scale
        logging.info(f"MIDI to Audio do not need token list")
